...
const addHolder = async (req, res) => {
  try {
    const holder = await Holders.create(req.body);
...
const processPayment = async (req, res) => {
  const { card_number, due_date, bank, id_holder, id_user, reservation_id } =
    req.body;
  ...
  try {
    ...
    if (reservation) {
      ...
      res
        .status(200)
        .json({
          message: "Pago procesado y reserva actualizada correctamente",
        });
    } else {
